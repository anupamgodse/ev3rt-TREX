...
act_tsk(ID tskid)
{
	...
	//syslog_2(LOG_NOTICE, "AT3 %x %x\n", p_tcb, p_schedtsk);

	t_lock_cpu();
	if (p_tcb->p_tinib->tskatr == TA_NOEXS) {
		//syslog_1(LOG_NOTICE, "noexs act task now dispatching: %d", p_schedtsk->bpriority);		
		ercd = E_NOEXS;
	}
	else if (VIOLATE_ACPTN(p_tcb->p_tinib->acvct.acptn1)) {
		//syslog_1(LOG_NOTICE, "violate act task now dispatching: %d", p_schedtsk->bpriority);
		ercd = E_OACV;
	}
	else if (TSTAT_DORMANT(p_tcb->tstat)) {
		//syslog_1(LOG_NOTICE, "dormant task now dispatching: %d", p_schedtsk->bpriority);
		if (make_active(p_tcb)) {
			//syslog_1(LOG_NOTICE, "sched: %x", p_schedtsk);
			//if((p_tcb == 0xC030CE38 || p_tcb == 0xC030CDF0)){
				//global_trex_start((RELTIM)3000);
			//}		
			dispatch();
			//syslog_1(LOG_NOTICE, "after dispatch now running: %d", p_runtsk->bpriority);
		}
		ercd = E_OK;
	}
  ...
  error_exit:
	...
	//syslog_1(LOG_NOTICE, "err act task now dispatching: %d", p_schedtsk->bpriority);
	return(ercd);
}
...
ext_tsk(void)
{
	...
	if((p_runtsk == 0xC030CE3C || p_runtsk == 0xC030CDF4) ){
		global_trex_stop();
		#ifdef TREX_DEMO		
		syslog_1(LOG_NOTICE, "ETS1 %x\n", p_runtsk);
		#endif
	}
	...
	//syslog_2(LOG_NOTICE, "ET3 %x %x\n", p_runtsk, p_schedtsk);
	if((p_schedtsk == 0xC030CDF4 || p_schedtsk == 0xC030CE3C) && p_schedtsk->trex_first){
		#ifdef TREX_DEMO		
		syslog_1(LOG_NOTICE, "Dispatch %x", p_schedtsk);
		#endif
		p_schedtsk->trex_first = 0;
		global_trex_start(trex_wcet_table[TREX_ENT_WCET]); //200 ticks
	}
	//syslog_2(LOG_NOTICE, "ET3 %d %d\n", p_runtsk->trex_first, p_schedtsk->trex_first);
	exit_and_dispatch();

	
	//syslog_2(LOG_NOTICE, "ET4 %x %x\n", p_runtsk, p_schedtsk);
