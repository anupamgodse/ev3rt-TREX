...
acre_cyc(const T_CCYC *pk_ccyc)
{
	...
	CHECK_RSATR(pk_ccyc->cycatr, TA_STA|TA_DOMMASK);
	CHECK_ATRDOMID_KERNEL(get_atrdomid(pk_ccyc->cycatr));
	...
	if (tnum_cyc == 0 || queue_empty(&free_cyccb)) {
		...
		p_cycinib->acvct.acptn1 = TACP_KERNEL;
		p_cycinib->acvct.acptn2 = TACP_KERNEL;
		p_cycinib->acvct.acptn3 = TACP_KERNEL | rundom;
		p_cycinib->acvct.acptn4 = TACP_KERNEL;
...
call_cychdr(CYCCB *p_cyccb)
{
	...
	//if(get_tcb_self(p_cyccb->p_cycinib->exinf) == 0xC030CE38 || get_tcb_self(p_cyccb->p_cycinib->exinf) == 0xC030CDF0){
		//syslog_1(LOG_NOTICE, "p_tcb: %x", p_cyccb->p_cycinib->exinf);
	//}
