 ...
 *  の責任を負わない．
 * 
 *  $Id: alarm.c 994 2014-04-18 08:48:40Z ertl-hiro $
 */
...
//bool_t global_trex_flag;

SYSTIM timo1;

SYSTIM timo2;

const T_HIRES_CALM p_hres_alm = {TA_NULL, 0, (ALMHDR)global_trex_handler};

const RELTIM trex_wcet_table[3] = {1,1,2};

TMEVTB global_trex_tmv;
uint8_t global_trex_status;
ID global_trex_id = 9;

uint8_t isr = 0;

//const HIRES_ALMCB* trex_cb = get_control_block(global_trex_id);

//bool_t trex_flag;

//#define TREX_DEMO

void global_trex_handler(void){
	syslog_0(LOG_NOTICE, "ALARM TRIGGERED\n");
	//global_trex_status = 2;
	p_schedtsk->trex_status = TREX_STATUS_TIMEOUT;
	//global_trex_flag = false;
	//assert(0==1);
}


void global_trex_start(RELTIM time){
	//assert(p_runtsk == p_schedtsk);
	#ifdef TREX_DEMO
	syslog_1(LOG_NOTICE, "ALARM Created %x\n", p_schedtsk);
	#endif
	//global_trex_status = 1;
	p_schedtsk->trex_status = TREX_STATUS_ACTIVE;
	
	//get_utm(&timo1);
	//syslog_1(LOG_NOTICE, "ALARM Created %lu", tm);
	sta_hires_alm(global_trex_id, time);	
	//global_trex_flag = 1;
	//global_trex_flag = true;
}

void global_trex_stop(void){
	if(p_schedtsk->trex_status == TREX_STATUS_ACTIVE){
		stp_hires_alm(global_trex_id);	
		p_schedtsk->trex_status = TREX_STATUS_INACTIVE;
		#ifdef TREX_DEMO
		syslog_0(LOG_NOTICE, "ALARM Stopped");
		#endif
	}else if(p_schedtsk->trex_status == TREX_STATUS_INACTIVE){
		#ifdef TREX_DEMO
		syslog_0(LOG_NOTICE, "ALARM Not Running");
		#endif
	}else if(p_schedtsk->trex_status == TREX_STATUS_TIMEOUT){
		#ifdef TREX_DEMO
		syslog_0(LOG_NOTICE, "ALARM Already Triggered");
		#endif
		p_schedtsk->trex_status = TREX_STATUS_INACTIVE;
	}else if(p_schedtsk->trex_status == TREX_STATUS_PAUSED){
		#ifdef TREX_DEMO
		syslog_0(LOG_NOTICE, "ALARM Paused");
		#endif
		p_schedtsk->trex_status = TREX_STATUS_INACTIVE;
	}	
}
 ...
 *  アラームハンドラの生成
 */
#ifdef TOPPERS_trex_start_alarm

ER_UINT
trex_start_alarm(RELTIM trex_wcet)
{
	global_trex_start(trex_wcet);
	return E_OK;
}

#endif /* TOPPERS_trex_start_alarm */

/*
 *  アラームハンドラのアクセス許可ベクタの設定
 */
