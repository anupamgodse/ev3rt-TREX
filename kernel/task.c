 ...
 *  の責任を負わない．
 * 
 *  $Id: task.c 994 2014-04-18 08:48:40Z ertl-hiro $
 */
...
initialize_task(void)
{
	...
	//p_hres_alm.almhdr = global_trex_handler;

	global_trex_id = acre_hires_alm(&p_hres_alm);
	...
	for (i = 0; i < TNUM_TPRI; i++) {
		...
		//syslog_2(LOG_NOTICE, "init: %d, %d", p_runtsk->bpriority, i+1);
	}
	ready_primap = 0U;
...
make_runnable(TCB *p_tcb)
{
	...
	//syslog_0(LOG_NOTICE, "MR");

	//if(p_tcb == 0xC030CE38 || p_tcb == 0xC030CDF0){
		//syslog_1(LOG_NOTICE, "MR %x\n", p_tcb);
	//}
	...
	if (p_schedtsk == (TCB *) NULL || pri < p_schedtsk->priority) {
		//if((p_tcb == 0xC030CE38 || p_tcb == 0xC030CDF0) && dspflg){
			//global_trex_start((RELTIM)3000);
		//}		
		//if(p_schedtsk!=NULL && p_schedtsk->trex_status == 1){
			//p_schedtsk->trex_store = tmevt_lefttim(&(p_schedtsk->trex_tmv));
			//tmevtb_dequeue(&(p_schedtsk->trex_tmv));
			//syslog_2(LOG_NOTICE, "%x switched %d left\n", p_schedtsk, tmevt_lefttim(&(p_schedtsk->trex_tmv)));
			//p_schedtsk->trex_status = 3; 
		//}
		p_schedtsk = p_tcb;
		return(dspflg);
	}
...
make_non_runnable(TCB *p_tcb)
{
	...
	if (queue_empty(p_queue)) {
		primap_clear(pri);
		if (p_schedtsk == p_tcb) {
			//temp = primap_empty() ? (TCB *) NULL : search_schedtsk();
...
make_active(TCB *p_tcb)
{
	...
	p_tcb->trex_first = 1;
	//syslog_2(LOG_NOTICE, "MA %x %d", p_tcb, p_tcb->trex_first);
	//syslog_flush();
	LOG_TSKSTAT(p_tcb);
...
rotate_ready_queue(uint_t pri)
{
	...
	//syslog_0(LOG_NOTICE, "SPIN TO WIN!!!\n");
	QUEUE	*p_queue = &(ready_queue[pri]);
	QUEUE	*p_entry;
