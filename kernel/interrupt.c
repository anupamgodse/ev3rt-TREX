 ...
 *  の責任を負わない．
 * 
 *  $Id: interrupt.c 994 2014-04-18 08:48:40Z ertl-hiro $
 */
...
call_isr(QUEUE *p_isr_queue)
{
	...
	if(p_schedtsk != NULL && p_schedtsk->trex_status == 1 && !isr){
		trex_cb = get_control_block(global_trex_id);		
		p_schedtsk->trex_store = ticks_to_usecs(trex_cb->left_ticks);//tmevt_lefttim(&(p_schedtsk->trex_tmv));
		stp_hires_alm(global_trex_id);						
		p_schedtsk->trex_status = TREX_STATUS_PAUSED; 
		p_schedtsk->trex_ict++;
		#ifdef TREX_DEMO
		syslog_0(LOG_NOTICE, "ITRS");
		#endif
	}
	...
	if(p_schedtsk != NULL && p_schedtsk->trex_status == 3 && !isr){
		#ifdef TREX_DEMO		
		syslog_0(LOG_NOTICE, "ITRE");
		#endif		
		p_schedtsk->trex_status = TREX_STATUS_ACTIVE;
		if(p_schedtsk->trex_ict == 5){
			p_schedtsk->trex_ict = 0;
			p_schedtsk->trex_store += trex_wcet_table[TREX_INT_WCET];
		}
		sta_hires_alm(global_trex_id, p_schedtsk->trex_store);		
	}
