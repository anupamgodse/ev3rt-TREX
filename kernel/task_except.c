 ...
 *  の責任を負わない．
 * 
 *  $Id: task_except.c 994 2014-04-18 08:48:40Z ertl-hiro $
 */
...
def_tex(ID tskid, const T_DTEX *pk_dtex)
{
	...
	//syslog_1(LOG_NOTICE, "DT %d", tskid);
	...
	t_lock_cpu();
	if (p_tinib->tskatr == TA_NOEXS) {
		ercd = E_NOEXS;
	}
	else if (VIOLATE_ACPTN(p_tinib->acvct.acptn3)) {
		ercd = E_OACV;
	}
...
ras_tex(ID tskid, TEXPTN rasptn)
{
	...
	//syslog_1(LOG_NOTICE, "RAS_T %d", tskid);
...
iras_tex(ID tskid, TEXPTN rasptn)
{
	...
	//syslog_1(LOG_NOTICE, "IT %d", tskid);
...
ref_tex(ID tskid, T_RTEX *pk_rtex)
{
	...
	//syslog_1(LOG_NOTICE, "RT %d", tskid);
